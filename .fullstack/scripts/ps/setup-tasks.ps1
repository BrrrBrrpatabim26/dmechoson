...
# setup-tasks.ps1 — Resolve tasks-template và in các thông tin cần thiết
# để AI agent render tasks.md (PowerShell variant).
#
# Tương đương spec-kit setup-tasks.ps1 đã rebrand:
#   - `.specify/` -> `.fullstack/`
...
if ($null -eq $tasksTemplateContent) {
    ...
    [Console]::Error.WriteLine("Template 'tasks-template' was not found in any supported location (overrides, presets, extensions, or shared core). Add an override at .fullstack/templates/overrides/tasks-template.md, or run 'crindex init' / reinstall shared infra to restore the core .fullstack/templates/tasks-template.md template.")
    exit 1
}
